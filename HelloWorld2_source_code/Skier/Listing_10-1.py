...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN or event.type == KEYDOWN:          # check for key presses
            if event.key == pygame.K_SPACE:        # left arrow turns left
                speed = skier.turn(-1)
            elif event.key == pygame.K_l:     #right arrow turns right
                speed = skier.turn(1)
